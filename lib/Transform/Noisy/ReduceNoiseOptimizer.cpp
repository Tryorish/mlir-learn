...
struct ReduceNoiseOptimizer : impl::ReduceNoiseOptimizerBase<ReduceNoiseOptimizer> {
    using ReduceNoiseOptimizerBase::ReduceNoiseOptimizerBase;

    void runOnOperation() { 
        Operation* module = getOperation();
        ReduceNoiseAnalysis analysis(module);
        // Use the int range analysis to confirm the noise is always below the maximum.
        DataFlowSolver solver;
        // The IntegerRangeAnalysis depends on DeadCodeAnalysis, but this dependence is not automatic and fails silently.
        solver.load<dataflow::DeadCodeAnalysis>();
        solver.load<dataflow::IntegerRangeAnalysis>();
        if (failed(solver.initializeAndRun(module)))
            signalPassFailure();
        auto result = module->walk([&](Operation *op) {
            if (!llvm::isa<noisy::AddOp, noisy::SubOp, noisy::MulOp, noisy::ReduceNoiseOp>(*op)) {
            ...
            if (!opRange || opRange->getValue().isUninitialized()) {
                op->emitOpError() << "Found op without a set integer range; did the analysis fail?";
                return WalkResult::interrupt();
            }
            ConstantIntRanges range = opRange->getValue().getValue();
            if (range.umax().getZExtValue() > MAX_NOISE) {
                op->emitOpError() << "Found op after which the noise exceeds the allowable maximum of " 
                                << MAX_NOISE 
                                << "; it was:"
